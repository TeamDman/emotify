...
namespace Emotify.Models
{
    public static class SeedData
    {
        ...
        private static async Task<EmotifyUser> EnsureUser(IServiceProvider serviceProvider, string testUserPw, string UserName)
        {
            var userManager = serviceProvider.GetService<UserManager<EmotifyUser>>();

            var user = await userManager.FindByNameAsync(UserName);
            if (user == null)
            {
                user = new EmotifyUser
                {
                    UserName = UserName,
                    EmailConfirmed = true
                };
